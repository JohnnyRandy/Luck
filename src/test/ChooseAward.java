		...
		//停止按钮的事件
		 ...
		 // 先产生三个三等奖，再两个二等奖，再一个一等奖，一个特等奖
			...
			case 7:
				...
				serial = chooseTime-6;
				message = "第" + serial + "位特等奖得主为：  " + str_name;
...
//							}
//							/*
//							 * 分别将分离开来的手机号码，和姓名分别放入两个可变向量当中 这里应该使用泛型
//							 * 捕获异常，看是否出现了格式错误，因为一般出现了格式错误 这两条语句都会发生无法取得元素的错误
//							 */
//							try {
//								v_identNumber.add(k, d[1]);
//								v_name.add(k, d[0]);
//								k++;
//							}
//
...
//							使用StringTokenizer来解析
							 ...
							 */
							try {
								v_identNumber.add(k, v.elementAt(1));
								v_name.add(k, v.elementAt(0));
								k++;
							}
